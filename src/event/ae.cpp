...
// poll fallback
...
#elif defined(AE_USE_KQUEUE)
    impl_->kq = kqueue();
    if (impl_->kq < 0) throw std::runtime_error(std::string("kqueue: ") + strerror(errno));
#endif
  }
  ...
  bool EventLoop::createFileEvent(int fd, int mask, FileEventCallback cb)
  {
    ...
    fe.mask |= mask;
    if (mask & AE_READABLE) fe.rfileProc = cb;
    if (mask & AE_WRITABLE) fe.wfileProc = cb;
    ...
    if (n)
    {
      if (kevent(impl_->kq, kev, n, nullptr, 0, nullptr) == -1) return false;
    }
#else
...
// map
  ...
  void EventLoop::deleteFileEvent(int fd, int mask)
  {
    ...
    if (it == impl_->fevents.end()) return;
    FileEvent& fe = it->second;
    fe.mask &= ~mask;
    if (mask & AE_READABLE) fe.rfileProc = nullptr;
    if (mask & AE_WRITABLE) fe.wfileProc = nullptr;
    if (fe.mask == AE_NONE) impl_->fevents.erase(it);
#ifdef AE_USE_EPOLL
  ...
  {
    auto it = impl_->fevents.find(fd);
    if (it == impl_->fevents.end()) return AE_NONE;
  ...
  void EventLoop::deleteTimeEvent(int64_t id)
  {
    ...
    vec.erase(std::remove_if(vec.begin(), vec.end(), [id](const TimeEvent& t) { return t.id == id; }), vec.end());
  }
  ...
  void EventLoop::run()
  {
    ...
    while (!impl_->stopFlag)
    {
      ...
      for (int i = 0; i < nfds; ++i)
      {
        ...
        int mask = 0;
        if (events[i].events & (EPOLLIN | EPOLLHUP | EPOLLERR)) mask |= AE_READABLE;
        if (events[i].events & EPOLLOUT) mask |= AE_WRITABLE;
        auto it = impl_->fevents.find(fd);
        if (it == impl_->fevents.end()) continue;
        FileEvent& fe = it->second;
        if ((mask & AE_READABLE) && fe.rfileProc) fe.rfileProc(fd, mask);
        if ((mask & AE_WRITABLE) && fe.wfileProc) fe.wfileProc(fd, mask);
      }
#elif defined(AE_USE_KQUEUE)
      timespec ts;
      ...
      int nfds = kevent(impl_->kq, nullptr, 0, events.data(), (int) events.size(), tsp);
      if (nfds < 0)
      {
        if (errno == EINTR) continue;
      ...
      for (int i = 0; i < nfds; ++i)
      {
        ...
        int mask = 0;
        if (events[i].filter == EVFILT_READ) mask |= AE_READABLE;
        if (events[i].filter == EVFILT_WRITE) mask |= AE_WRITABLE;
        auto it = impl_->fevents.find(fd);
        if (it == impl_->fevents.end()) continue;
        FileEvent& fe = it->second;
        if ((mask & AE_READABLE) && fe.rfileProc) fe.rfileProc(fd, mask);
      ...
      for (auto& p : impl_->fevents)
      {
        ...
        pd.fd     = p.first;
        pd.events = 0;
        if (p.second.mask & AE_READABLE) pd.events |= POLLIN;
        if (p.second.mask & AE_WRITABLE) pd.events |= POLLOUT;
        pfds.push_back(pd);
      }
      ...
      for (auto& pd : pfds)
      {
        if (pd.revents == 0) continue;
        int mask = 0;
        if (pd.revents & (POLLIN | POLLHUP | POLLERR)) mask |= AE_READABLE;
        if (pd.revents & POLLOUT) mask |= AE_WRITABLE;
        auto it = impl_->fevents.find(pd.fd);
        if (it == impl_->fevents.end()) continue;
        FileEvent& fe = it->second;
        if ((mask & AE_READABLE) && fe.rfileProc) fe.rfileProc(pd.fd, mask);
        if ((mask & AE_WRITABLE) && fe.wfileProc) fe.wfileProc(pd.fd, mask);
      }
#endif
      // process time events
      now = mstime();
