...
namespace cppkit::event
{
  ...
  TcpServer::TcpServer(EventLoop& loop, const std::string& addr, uint16_t port) : loop_(loop), addr_(addr), port_(port)
  {
  }
  ...
  void TcpServer::start()
  {
    ...
    loop_.createFileEvent(listenfd_, AE_READABLE,
        [this](int fd, int mask)
        {
          while (true)
          {
            ...
            loop_.createFileEvent(c, AE_READABLE,
                [this](int cfd, int cmask)
                {
                  // read data
                  char buf[4096];
                  ...
                  if (n > 0)
                  {
                    if (onMsg_) onMsg_(connections_[cfd], std::vector<uint8_t>(buf, buf + n));
                  }
                  else
                  {
                    // closed or error
