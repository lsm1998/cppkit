...
using FileEventCallback =
    std::function<void(int fd, int mask)>; // mask: AE_READABLE|AE_WRITABLE
using TimeEventCallback = std::function<int64_t(
    int64_t id)>; // return next when >0, else -1 to cancel
