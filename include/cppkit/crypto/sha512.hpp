...
namespace cppkit::crypto
{
  ...
  private:
    ...
    static constexpr std::array<uint64_t, 80> K = {0x428a2f98d728ae22ULL,
  ...
  inline void SHA512::update(const uint8_t* data, size_t len)
  {
    size_t i = 0;
    while (i < len)
    {
  ...
  inline void SHA512::finalize()
  {
    if (finalized_)
      return;
    uint64_t totalBits = bitLen_ + bufferLen_ * 8;

    // Padding
    size_t padLen = (bufferLen_ < 112) ? (112 - bufferLen_) : (240 - bufferLen_);
    std::array<uint8_t, 256> pad{};
    pad[0] = 0x80;
    uint64_t highBits = 0; // 高位为 0，支持消息长度 < 2^64 bits
    uint64_t lowBits = totalBits;
    for (int i = 0; i < 8; ++i)
    {
  ...
  inline void SHA512::transform(const uint8_t block[128])
  {
    uint64_t w[80];
    for (int i = 0; i < 16; ++i)
    {
      w[i] = (uint64_t(block[i * 8]) << 56) | (uint64_t(block[i * 8 + 1]) << 48) | (uint64_t(block[i * 8 + 2]) << 40) |
             (uint64_t(block[i * 8 + 3]) << 32) | (uint64_t(block[i * 8 + 4]) << 24) |
             (uint64_t(block[i * 8 + 5]) << 16) | (uint64_t(block[i * 8 + 6]) << 8) | uint64_t(block[i * 8 + 7]);
    }
    for (int i = 16; i < 80; ++i)
    {
  ...
  inline std::array<uint8_t, 64> SHA512::digest()
  {
    finalize();
  ...
  inline std::string SHA512::hexDigest()
  {
    auto d = digest();
