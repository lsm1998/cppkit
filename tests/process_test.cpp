...
int main()
{
    ...
    pm.spawn("ping", {"-c", "3", "127.0.0.1"}, [](pid_t _, const int code)
    {
        if (code == 0)
    ...
    // 测试杀死进程
    const pid_t sleepPid = pm.spawn("sleep", {"10"}, [](pid_t _, const int code)
    {
    ...
    // 测试长时间运行的进程
    pm.spawn("sleep", {"50"}, [](pid_t _, const int code)
    {
        cppkit::sync_stream(std::cout) << "长时间 sleep 进程结束，code: " << code << "\n";
    });
